...
const InvestorPitchDeckPage = () => {
  ...
  const [activePanel, setActivePanel] = useState<string>('pitch');
          ...
          <div className={styles.switchBody} role="tabpanel">
            <div className={styles.report}>
              <div className={styles.reportHeader}>
                <p className={styles.reportTitle}>Codebase Analysis Report</p>
                <p className={styles.reportMeta}>
                  Generated by Code Visualizer from production repos. “Broken connections” often represent auth-gated edges and are not treated as broken runtime paths.
                </p>
              </div>

              <div className={styles.reportSection}>
                <p className={styles.reportSectionTitle}>Overview</p>
                <div className={styles.reportTable} role="table" aria-label="Codebase overview">
                  <div className={styles.reportRow} role="row">
                    <div className={styles.reportCellKey} role="cell">Frontend</div>
                    <div className={styles.reportCellVal} role="cell">
                      4,514 nodes • 970 files • 2,384 functions
                    </div>
                  </div>
                  <div className={styles.reportRow} role="row">
                    <div className={styles.reportCellKey} role="cell">Backend</div>
                    <div className={styles.reportCellVal} role="cell">
                      29,877 nodes • 1,696 files • 17,613 functions • 6,166 classes
                    </div>
                  </div>
                  <div className={styles.reportRow} role="row">
                    <div className={styles.reportCellKey} role="cell">API surface</div>
                    <div className={styles.reportCellVal} role="cell">4,338 enumerated endpoints</div>
                  </div>
                </div>
              </div>

              <div className={styles.reportSection}>
                <p className={styles.reportSectionTitle}>Traceability</p>
                <p className={styles.reportParagraph}>
                  Every endpoint is mapped to the functions, classes, and external services it touches. This enables deterministic audit trails: API request → service boundary → internal call graph → downstream dependencies.
                </p>
              </div>

              <div className={styles.reportSection}>
                <p className={styles.reportSectionTitle}>Dependencies</p>
                <p className={styles.reportParagraph}>
                  Backend graph includes ~58,498 inter-node connections across 40 services. The dependency model treats auth-gated routes as intentional boundaries (not failures).
                </p>
              </div>

              <div className={styles.reportSection}>
                <p className={styles.reportSectionTitle}>Pipeline</p>
                <p className={styles.reportParagraph}>
                  Pipeline view is built from the gateway routing layer and service routers, producing a map of ingress → policy/auth → service handlers → storage/queues.
                </p>
              </div>

              <div className={styles.reportSection}>
                <p className={styles.reportSectionTitle}>Heat Map</p>
                <p className={styles.reportParagraph}>
                  Heat is derived from fan-in/fan-out connectivity and endpoint density. Highest-heat zones typically include: gateway routers, orchestration/services layers, and shared policy/auth middleware.
                </p>
              </div>

              <div className={styles.reportFooter}>
                <p className={styles.reportFootnote}>
                  Next: we’ll attach the full CV governance + pipeline JSON outputs and render top dependencies + hotspots directly in this report.
                </p>
              </div>
            </div>
          </div>
    ...
    const interval = window.setInterval(() => {
      setActivePanel((p) => {
        const currentIndex = Math.max(
          0,
          panels.findIndex((panel) => panel.id === p)
        );
          ...
          <div className={styles.heroGrid}>
            <div className={styles.heroContent}>
              ...
              <div className={styles.switchPanel}>
                <div className={styles.switchTabs} role="tablist" aria-label="Investor pitch details">
                  {panels.map((panel) => (
                    <button
                      ...
                      className={`${styles.switchTab}${activePanel === panel.id ? ` ${styles.switchTabActive}` : ''}`}
                ...
                {panels.find((p) => p.id === activePanel)?.render() ?? panels[0].render()}
              </div>
